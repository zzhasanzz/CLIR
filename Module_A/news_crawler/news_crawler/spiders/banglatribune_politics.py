...
class BanglaTribunePoliticsSpider(scrapy.Spider):
    name = "banglatribune_politics"
    ...
    def parse(self, response):
        # extract and queue the first page links
    ...
    def parse_api(self, response):
        ...
        if not html:
            self.logger.warning("No HTML in Politics API response")
            return

        selector = scrapy.Selector(text=html)
        links = self.extract_links(selector)

        self.logger.info("Politics AJAX links: %d", len(links))
        yield from self.queue_links(response, links)

        total = response.meta["total"] + len(links)
        start = response.meta["start"]

        # continue pagination until ~100 articles
        if total < 100 and links:
            ...
            yield scrapy.Request(
                ...
                headers={
                    "X-Requested-With": "XMLHttpRequest",
                    "Referer": "https://www.banglatribune.com/politics",
                    "Accept": "application/json, text/javascript, */*; q=0.01"
                }
            )
