...
class DailyStarEconomySpider(scrapy.Spider):
    name = "thedailystar_economy"
    ...
    def parse(self, response):
        ...
        # IMPORTANT: start AJAX from page=1 (page=0 duplicates initial HTML)
    ...
    def parse_ajax(self, response):
        ...
        # 🔥 IMPORTANT FIX: extract from viewsShowMore
        html_parts = [
            item.get("data", "")
            for item in data
        ...
        yield scrapy.Request(
            ...
            headers={
                "X-Requested-With": "XMLHttpRequest",
                "Accept": "application/json",
                "Referer": "https://www.thedailystar.net/business/economy"
            }
        )
    ...
    def extract_links(self, selector):
        return selector.css(
            ...
            "a[href^='/business/economy/news/']"
            "::attr(href)"
        ).getall()

    # --------------------------------------------------
    # QUEUE ARTICLES (DEDUP + LIMIT)
    # --------------------------------------------------
    ...
    def parse_article(self, response):
        ...
        # Extract visible paragraph text only
    ...
    def build_ajax_url(self, page):
        return (
            ...
            "&view_args=159361"
            "&view_path=%2Ftaxonomy%2Fterm%2F159361"
            "&pager_element=0"
            f"&page={page}"
            "&_drupal_ajax=1"
        )
