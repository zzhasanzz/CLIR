...
class BanglaTribuneNationalSpider(scrapy.Spider):
    name = "banglatribune_national"
    ...
    def parse_api(self, response):
        ...
        self.logger.info("New NATIONAL links from API: %d", len(links))
        yield from self.queue_links(response, links)
        ...
        if total < 100 and links:
            ...
            yield scrapy.Request(
                ...
                headers={
                    "X-Requested-With": "XMLHttpRequest",
                    "Referer": "https://www.banglatribune.com/national",
                    "Accept": "application/json, text/javascript, */*; q=0.01"
                }
            )
