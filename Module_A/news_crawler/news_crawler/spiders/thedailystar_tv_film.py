...
class DailyStarTVFilmSpider(scrapy.Spider):
    name = "thedailystar_tv_film"
    allowed_domains = ["thedailystar.net"]

    start_urls = [
        "https://www.thedailystar.net/entertainment/tv-film"
    ]
    ...
    def parse(self, response):
        ...
        # Start AJAX pagination from page=1
    ...
    def parse_ajax(self, response):
        ...
        # 🔥 extract HTML from viewsShowMore only
        html_parts = [
            item.get("data", "")
            for item in data
        ...
        yield scrapy.Request(
            ...
            headers={
                "X-Requested-With": "XMLHttpRequest",
                "Accept": "application/json",
                "Referer": "https://www.thedailystar.net/entertainment/tv-film"
            }
        )

    # --------------------------------------------------
    # LINK EXTRACTION (CORRECT & GENERIC)
    # --------------------------------------------------
    ...
    def build_ajax_url(self, page):
        return (
            ...
            "&view_args=283453"
            "&view_path=%2Ftaxonomy%2Fterm%2F283453"
            "&pager_element=0"
            f"&page={page}"
            "&_drupal_ajax=1"
        )
